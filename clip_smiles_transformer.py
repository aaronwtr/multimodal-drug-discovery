...
def setup_args():
    ...
    options.add_argument('--dataset', action="store", default="cell-painting-smiles")
...
####################################  Transformer test with smiles using x-clip based transformers #################################
...
args.dataset='cell-painting-smiles'
...
for epoch in range(epochs):
    ...
    for batch_idx, (real_sample, cond,tokenized_mol) in enumerate(trainloader): 
        
        
        
        real_sample=real_sample.to(device)
        tokenized_mol=tokenized_mol.to(device)


        optimizer.zero_grad()
        loss=clip(tokenized_mol,real_sample,return_loss=True)
        loss.backward()
        optimizer.step()
